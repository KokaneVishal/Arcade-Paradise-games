...
window.onload = function () {
	highscore = localStorage.getItem("pong_key");
}
...
function update() {
	...
	if (collides(ball, p1)) {
		...
		if (ball.y + ball.r > H) {
			ball.y = H - ball.r;
			var snd = new Audio("img/punch.wav");
			snd.play();
			gameOver();
		}
		...
		if (ball.x + ball.r > W) {
			...
			var snd = new Audio("img/wall.mp3");
			snd.play();
		}
...
function collideAction(ball, p) {
	...
	if (paddleHit == 1) {
		...
		var snd = new Audio("img/ding.wav");
		snd.play();
	}
...
function updateScore() {
	...
	ctx.fillText("Your High Score : " + highscore, 20, 60);
	if (points > highscore) {
		highscore = points;
		getpong_score = highscore.toString();
		localStorage.setItem("pong_key", getpong_score);
	}
